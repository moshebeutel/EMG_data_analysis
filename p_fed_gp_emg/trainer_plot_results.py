...
parser.add_argument("--results-folder", type=str, default='./shallow_learn_results')
parser.add_argument("--file-name", type=str, default='classification_result_putEMG_pFedGP-Full_seed_42_wd_0.001_lr_0.01_num_steps_400_inner_steps_1_objective_predictive_likelihood_predict_ratio_0.5_24chn.bin')

args = parser.parse_args()

working_directory = args.results_folder
...
if 1:
    output: Dict[str, any] = pickle.load(
        open(os.path.join(working_directory, args.file_name), "rb"))

    output2: Dict[str, any] = pickle.load(
    ...
    index = np.arange(len(output["classifiers"]) + len(output2["classifiers"]))
    bar_width = 0.15
    bar_spacer = 0.025
    ...
    for i, feature_set in enumerate(output["feature_sets"].keys()):
        ...
        classifiers = list(output["classifiers"].keys()) + list(output2["classifiers"].keys())
        for i_clf, clf in enumerate(classifiers):
            data = list(filter(lambda r: r["clf"] == clf and r["feature_set"] == feature_set, output["results"]))
            data2 = list(filter(lambda r: r["clf"] == clf and r["feature_set"] == feature_set, output2["results"]))
    ...
    for f, a in zip(figs, axes):
        ...
        a.set_ylim([y_lim[met], 1.01])
        a.set_xticks(index + ((bar_width + bar_spacer) * (len(output["feature_sets"]) - 1)) / 2)
        a.set_xticklabels(list(output["classifiers"].keys()) + list(output2["classifiers"].keys()))
...
if 1:
    ...
    classifier_feature_sets = [("pFedGP", "Du"), ("pFedGP", "Hudgins"), ("pFedGP", "RMS")]
