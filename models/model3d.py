...
class RawEmg3DConvnet(nn.Module):
    def __init__(self, number_of_classes, window_size=1280, depthwise_multiplier=32, W=3, H=8, logger=None):
        ...
        logger.info(f"Number Parameters: {self.get_n_params()}")
    ...
    def initialize_weights(self):
        self.logger.debug('initialize_weights')
        for m in self.modules():
            self.logger.debug(f'initialize {m}')
            if isinstance(m, nn.Conv3d):
                self.logger.debug(f'before w initialize {m}')
                torch.nn.init.kaiming_normal_(m.weight)
                # self.logger.debug(f'after w initialize {m.weight}')
                # self.logger.debug(f'before b initialize {m.bias.data}')
                m.bias.data.zero_()
                # self.logger.debug(f'after b initialize {m.bias.data}')
    ...
    def forward(self, x):
        ...
        if self._window:
            conv2 = self._prelu2(self._batch_norm2(self._conv2(pool1.squeeze())))
            ...
            conv3 = self._prelu3(conv3)
            # conv3 = self._dropout3(self._prelu3(conv3))
            self.logger.debug(f'prelu3 {conv3.shape}')
...
if __name__ == '__main__':
    ...
    model = RawEmg3DConvnet(number_of_classes=64, window_size=1, depthwise_multiplier=32, logger=logging.getLogger())
